...
// parse parses a hexadecimal substring s into an unsigned integer of the given bitSize.
// It returns the parsed value and true on success, or 0 and false on failure (empty input or invalid hex).
...
// findValid scans a hex-encoded string for an ICMP extension header marker and returns the substring from the marker onward.
// 
// findValid requires hexStr to be an even-length string with at least 8 hex characters. It searches from the end toward the
// beginning in 4-byte (8-hex-digit) steps for the marker "2000". If found and the data following the 4-byte header contains
// at least 8 bytes, findValid returns the substring beginning at the marker; otherwise it returns an empty string.
...
// extractMPLS extracts MPLS Label Stack Entry descriptions from an ICMP message.
// It returns a slice of strings formatted as "[MPLS: Lbl <label>, TC <tc>, S <s>, TTL <ttl>]" for each parsed LSE,
// or nil if MPLS parsing is disabled, no valid MPLS extension is present, or parsing/validation fails.
