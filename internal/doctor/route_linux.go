...
func linuxRouteRequest(method trace.Method, cfg trace.Config, seq uint32) ([]byte, error) {
	ip := cfg.DstIP.To4()
...
func queryRoute(ctx context.Context, method trace.Method, cfg trace.Config) (Route, error) {
	r := Route{Conditions: routeConditions(method, cfg) + fmt.Sprintf(" uid=%d", os.Geteuid())}
	if cfg.SrcPort == 0 && method != trace.ICMPTrace {
		r.Limitations = "source port selected during probing; ECMP prediction may differ"
	}
	if err := ctx.Err(); err != nil {
		return r, err
	}
	b, err := linuxRouteRequest(method, cfg, 1)
	if err != nil {
		return r, err
	}
	...
	for {
		...
		for _, m := range msgs {
			...
			if m.Header.Type == unix.RTM_NEWROUTE {
				return parseLinuxRoute(m, r)
			}
		}
	}
}

func parseLinuxRoute(m syscall.NetlinkMessage, r Route) (Route, error) {
	if len(m.Data) < 12 {
